...
After you have done the pull, you may need to:

* Rename all the columns and tables to lowercase (so your active record attribs work as usual and so rails can see the schema_migrations table).
...
* Convert any text columns to varchar(8000), truncating where necessary, (so that those columns are comparable by MSSQL):
